  ...
  return (
    <div className="w-full bg-card border border-border/80 rounded-2xl p-4 sticky left-0 top-28 shadow-xs font-inter transition-all duration-300 space-y-6">
      ...
      <div className="flex items-center gap-3 px-2 py-3 border-b border-border/60">
        <div className="h-10 w-10 rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center text-primary font-black text-sm select-none">
      ...
      <ul className="space-y-1">
        {navList.map((nav, i) => {
          ...
          return (
            <li key={i}>
              <Link
                href={nav.href}
                className={`flex items-center px-4 py-2.5 gap-3 rounded-xl text-xs font-black uppercase tracking-wider transition-all duration-200 select-none cursor-pointer ${
                  active
        ...
        <li className="pt-2 mt-2 border-t border-border/60">
          <button
            onClick={() => logout()}
            className="w-full flex items-center px-4 py-2.5 gap-3 rounded-xl text-xs font-black uppercase tracking-wider text-rose-500 hover:text-rose-600 hover:bg-rose-500/10 transition-all duration-200 select-none cursor-pointer"
          >
