...
import { ShieldCheck, Truck, Sparkles, AlertCircle, RefreshCcw } from "lucide-react";
...
export const generateMetadata = async ({ params }: IPackage) => {
  const { slug } = await params;
  const pkg = PACKAGES_DATA.find((p) => p.slug === slug);
  return {
    title: `${pkg?.name || "Solar Package"} | GoSolar`,
    description: pkg?.desc,
  };
};

const PackagePageContent = async ({ slug }: { slug: string }) => {
  const pkg = PACKAGES_DATA.find((p) => p.slug === slug);
  ...
  // Load capabilities mockup based on inverter range size
  const inverterValue = pkg.inverterRange.toLowerCase();
  const loadCapabilities = inverterValue.includes("10")
    ? [
      ...
      <div className="w-full bg-zinc-50/50 dark:bg-zinc-900/10 border-b border-zinc-150 dark:border-zinc-850 py-3.5">
        <div className="container mx-auto px-4 max-w-6xl">
          <nav className="flex flex-wrap items-center gap-1.5 text-xs font-semibold text-zinc-400 dark:text-zinc-500">
            ...
            <Link href="/packages" className="hover:text-zinc-950 dark:hover:text-white transition-colors">
              Packages
            </Link>
            <span className="text-zinc-300 dark:text-zinc-700 select-none">/</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
            ...
            <div className="lg:col-span-7 space-y-8">
              ...
              <div className="w-full">
                <ProductImages
                  images={pkg.constituents.map((item) => ({
                    url: item.imageUrl,
                    public_id: item.id,
                  }))}
                />
              ...
              <div className="bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-800 rounded-3xl p-6 md:p-8 space-y-4">
                ...
                <p className="text-xs text-zinc-505 dark:text-zinc-400 leading-relaxed">
                  Below is the recommended loading design to operate comfortably with this configuration:
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 pt-2">
                  {loadCapabilities.map((item, i) => (
                    <div key={i} className="flex items-start gap-2.5 text-xs text-zinc-650 dark:text-zinc-350">
            ...
            <div className="lg:col-span-5 space-y-6">
              <div className="bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-800 rounded-3xl p-6 md:p-8 shadow-xs space-y-6">
                ...
                <div className="space-y-2 pb-4 border-b border-zinc-100 dark:border-zinc-800">
                  <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider border ${pkg.badgeColor}`}>
                    {pkg.inverterRange} Solar Setup
                  </span>
                ...
                <div className="space-y-3">
                  <h4 className="text-[10px] font-extrabold uppercase text-zinc-400 tracking-wider">Included Hardware & Services</h4>
                  <div className="bg-zinc-50 dark:bg-zinc-800/40 p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800 space-y-3">
                    {pkg.spec.split(" + ").map((spec, i) => (
                      <div key={i} className="flex items-center gap-2.5 text-xs font-semibold text-zinc-700 dark:text-zinc-300">
                ...
                <div className="space-y-1">
                  <span className="text-[9px] uppercase font-bold text-zinc-400 tracking-wider">Installed System Cost</span>
                ...
                <div className="space-y-2 pt-4 border-t border-zinc-100 dark:border-zinc-800">
                  <h4 className="text-xs font-bold text-zinc-700 dark:text-zinc-200">Good to know</h4>
                  <h5 className="text-[11px] font-bold text-zinc-900 dark:text-white">
                    Onsite Setup &amp; Cabling Included
                  </h5>
                  <p className="text-[10px] text-zinc-505 dark:text-zinc-450 leading-relaxed">
                    This solar configuration is optimized for complete installation. Our certified engineers will carry out onsite assessments, changeover configuration, surge wiring, and test runs completely free.
                  </p>
                </div>
              ...
              <div className="bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-800 rounded-3xl p-6 space-y-4">
                <h4 className="text-xs font-bold text-zinc-900 dark:text-white">Delivery &amp; Setup Info</h4>
                <div className="space-y-3.5">
                  <div className="flex gap-3 items-start">
                    <Truck className="h-5 w-5 text-primary shrink-0 mt-0.5" />
                    <div className="space-y-0.5">
                      <h5 className="font-semibold text-xs text-zinc-800 dark:text-zinc-200">Nigeria Delivery Coverage</h5>
                      <p className="text-[10px] text-zinc-500 dark:text-zinc-400 leading-relaxed">
                        Free delivery and onsite technicians dispatch within Lagos, Port Harcourt, and Uyo experience center ranges (4-5 working days standard).
                      </p>
                    </div>
                  </div>
                  <div className="flex gap-3 items-start">
                    ...
                    <div className="space-y-0.5">
                      <h5 className="font-semibold text-xs text-zinc-800 dark:text-zinc-200">Warranty Protection</h5>
                      <p className="text-[10px] text-zinc-500 dark:text-zinc-400 leading-relaxed">
                        Complete support coverage including 25 years warranty on solar panels and 2-5 years replacement policies on lithium batteries.
                      </p>
                    </div>
                  </div>
                </div>
