...
const PackagePage = () => {
  ...
  const { data: pkg, isLoading, error } = usePackageByIdQuery(slug);
  ...
  const loadCapabilities = pkg.powers && pkg.powers.length > 0
    ? pkg.powers
    : pkg.capacityKva >= 10
    ? [
  ...
  const packageImages = pkg.constituents && pkg.constituents.length > 0
    ? pkg.constituents
        .map((c: any) => {
  ...
  const finalImages = packageImages.length > 0 ? packageImages : [{ url: "/images/bg/hero-bg.jpg", public_id: "hero" }];
            ...
            <div className="lg:col-span-5 space-y-6">
              <div className="bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-800 rounded-3xl p-6 md:p-8 shadow-xs space-y-6">
                ...
                <div className="space-y-2 pb-4 border-b border-zinc-100 dark:border-zinc-800 font-semibold">
                  <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider border ${badgeColor}`}>
                ...
                <div className="space-y-3 font-semibold">
                  ...
                  <div className="bg-zinc-50 dark:bg-zinc-800/40 p-4 rounded-2xl border border-zinc-150 dark:border-zinc-800 space-y-3">
                    ...
                    {pkg.highlights && pkg.highlights.map((highlight: string, i: number) => (
                      <div key={i} className="flex items-center gap-2.5 text-xs font-semibold text-zinc-700 dark:text-zinc-300">
                        <ShieldCheck className="h-4.5 w-4.5 text-emerald-600 dark:text-emerald-400 shrink-0" />
                        <span>{highlight}</span>
                      </div>
